/**
 * Diff Esther Gomes's f30 (aged ghost) against Eliza Pancakes's f30 (aged
 * human) to find the ghost-specific fields. Also dump Casper NewGhost's f30
 * for context (fresh CAS ghost, only 36 bytes).
 */
...
const SAMPLES = [
  ...
  { kind: 'aged-human',  first: 'Eliza',  last: 'Pancakes' },
  { kind: 'aged-human2', first: 'Bella',  last: 'Goth' },
...
function decodeMsg(start, end, indent, maxFields) {
  let p = start, i = 0;
...
for (const s of SAMPLES) {
  ...
  if (!r) { console.log(`\nSKIP: ${s.kind} (${s.first} ${s.last})`); continue; }
  const f30Pos = findField(r.bodyStart, r.bodyEnd, 30, 2);
  if (f30Pos === null) { console.log(`\n${s.kind}: no f30`); continue; }
  const [f30Len, f30Start] = readV(data, f30Pos);
  const f30End = f30Start + Number(f30Len);
  console.log(`\n══════════ ${s.kind.toUpperCase()} (${s.first} ${s.last}) — f30 is ${f30Len} bytes ══════════`);
  decodeMsg(f30Start, f30End, '  ', 80);
}
