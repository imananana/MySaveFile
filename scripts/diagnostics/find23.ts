...
function walk(buf: Uint8Array, path: string) {
  ...
  while (q < buf.length) { let fn, wire, at; try { [fn, wire, at] = readTag(buf, q); } catch { break; } if (fn === 0) break; q = at;
    if (wire === 0) { const [v, n] = readVarint(buf, q); q = n; if (v===23n||v===10n) console.log(`  ${path}.f${fn} = 23 (varint)`); }
    else if (wire === 1) { const dv=new DataView(buf.buffer,buf.byteOffset+q,8); if(readFixed64LE(buf,q)===23n||dv.getFloat64(0,true)===23) console.log(`  ${path}.f${fn} = 23 (fixed64)`); q += 8; }
    else if (wire === 5) { const dv=new DataView(buf.buffer,buf.byteOffset+q,4); if(dv.getInt32(0,true)===23||dv.getFloat32(0,true)===23) console.log(`  ${path}.f${fn} = 23 (fix32 ${dv.getFloat32(0,true)===23?'float':'int'})`); q += 4; }
...
for (const rec of iterLDFields(svc, 1)) {
  ...
  console.log('Searching Tester Business record for value 23:');
  walk(rec, 'rec');
  console.log('(done)');
}
