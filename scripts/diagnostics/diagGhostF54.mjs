/**
 * Compare f54-f60 varint values for ghosts vs humans (no width truncation).
 */
...
const GHOSTS = [['Esther', 'Gomes'], ['Consort', 'Capp'], ['Felix', 'Psyded']];
const HUMANS = [
  ['Kyle', 'Kyleson'], ['Eliza', 'Pancakes'], ['Bob', 'Pancakes'],
  ['Bella', 'Goth'], ['Cassandra', 'Goth'],
];
...
function getVarintField(start, end, targetFn) {
  const targetTag = (targetFn << 3) | 0;
  let p = start;
  while (p < end) {
    let tag;
    const b0 = data[p];
...
const targetFields = [54, 55, 56, 57, 58, 59, 60, 65, 66, 67, 68, 70, 71, 72, 73, 74, 75];
console.log('SAMPLE                    ' + targetFields.map(f => `f${f}`.padStart(14)).join(''));
console.log('─'.repeat(26 + 14 * targetFields.length));
for (const s of samples) {
  const r = findBody(s.first, s.last);
  if (!r) { console.log(`${s.kind} ${s.first} ${s.last}: NOT FOUND`); continue; }
  const row = [`${s.kind} ${s.first} ${s.last}`.padEnd(26)];
  for (const fn of targetFields) {
    const v = getVarintField(r.bodyStart, r.bodyEnd, fn);
    row.push((v === null ? '·' : v.toString()).padStart(14).slice(-14));
  }
